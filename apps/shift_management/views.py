...
def shift_list_view(request):
    ...
    # Get shifts with available slots
    shifts = Shift.objects.filter(
        status='open',
        shift_date__gte=timezone.now().date()
    ).annotate(
        approved_volunteers=Count('volunteers', filter=Q(volunteers__status='approved'))
    ).select_related('store', 'manager')
    
    # Filter to only show shifts with available slots
    available_shifts = []
    for shift in shifts:
    ...
    if role_filter:
        available_shifts = [s for s in available_shifts if s.role_required == role_filter]
    
    store_filter = request.GET.get('store')
    if store_filter:
        available_shifts = [s for s in available_shifts if str(s.store.id) == store_filter]
